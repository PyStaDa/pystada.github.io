...
def main():
    def format_date(date, hide_agenda_link=False):
        if hide_agenda_link:
            line_template = '<li><span style="color: #999999;">Mittwoch, {date.day} {month} {date.year} um 19:30 Uhr <!--(<a href="https://pads.ccc.de/pystada-{date.year}-{date.month}-{date.day}">Agenda</a>)--></span></li>'
        else:
            line_template = '<li><span>Mittwoch, {date.day} {month} {date.year} um 19:30 Uhr (<a href="https://pads.ccc.de/pystada-{date.year}-{date.month}-{date.day}">Agenda</a>)</span></li>'
